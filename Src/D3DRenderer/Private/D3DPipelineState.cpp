...
D3DPipelineState::D3DPipelineState(const D3DRootSignature* root_signature, const D3DVertexShader* vertex_shader, const D3DPixelShader* pixel_shader) :
    root_signature_(root_signature), vertex_shader_(vertex_shader), pixel_shader_(pixel_shader)
{
}
...
void D3DPipelineState::Compile(ID3D12Device2* device)
{
    ...
    state.pRootSignature = root_signature_->root_signature_;
    state.InputLayout = { root_signature_->ied_, _countof(root_signature_->ied_) };
    ...
    d3dAssert(device->CreatePipelineState(&desc, IID_PPV_ARGS(&pipeline_state_)));
}
